...
(思路：**方法一（子数组是在中间，和最大子数组和情况相同；或者子数组在首尾，固定后缀j，遍历前缀和0-j-1)**）
```
*****方法一
class Solution {
public:
...
```
（**方法二：同时记录连续最大子数组和，以及连续最小子数组和，首位情况=总和-最小连续**）
```
class Solution {
public:
...
(**注意二分搜索的边界情况，可以设定为左闭右开区间，通过边界的情况调整**)
```
***left bound
int left_bound(vector<int>& nums, int target) {
    int left = 0;
    int right = nums.size(); // 注意,左闭右开，所以从n开始
    
    while (left < right) { // 注意
        int mid = left + (right - left) / 2;
        if (nums[mid] == target) {
            right = mid;//找到第一个左值，所以right=mid；
        } else if (nums[mid] < target) {
            left = mid + 1; //左闭右开，所以mid搜索过了，left=mid+1
        } else if (nums[mid] > target) {
            right = mid; // 注意，右开，因此mid没有搜索过，right=mid
        }
    }
    return left;
}
...
public:
    int findMin(vector<int>& nums) {
        ...
        while(l<r){  
            ...
            // cout << mid << endl;
            if(nums[mid]<nums[r]){
                r = mid; //如果中间的值小于最右，则收缩有边界，但是有可能右边是最小值，所以r=mid；
            }else{
               l = mid+1;//如果中值大于等于右边界，最小值在右边，则收缩左边界，因为最小值一定不是mid，所以，l=mid+1；
            }
        }
        cout << l << endl;
        return nums[l];
...
(思路：贪心算法，每次选择最大收益的选项，在w满足启动条件的情况下)
```
class Solution {
public:
...
（思路：维护两个堆，大顶和小顶堆，每次入堆需要判断哪个堆的元素更多，把多的元素出堆，然后再入堆，保证大堆的最小值大于小堆的最大值
...
(思路：初始化把nums1的前k个索引（0，1)，（1，0）...入队，每次更新nums2的序列进去）
```
class Solution {
public:
...
(思路：一个数和它本身做异或运算结果为 0，即 a ^ a = 0；一个数和 0 做异或运算的结果为它本身，即 a ^ 0 = a。)
```
class Solution {
public:
